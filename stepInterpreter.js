...
function interpretStep(stepDescription, testData) {
  ...
  if (matchesAny(text, ['click', 'press', 'tap', 'submit', 'hit'])) {
    return { type: 'click', target: cleanTarget(stepDescription), value: null };
  }

  if (matchesAny(text, ['enter', 'type', 'input', 'fill in', 'write'])) {
    return { type: 'type', target: cleanTarget(stepDescription), value: testData || extractQuoted(stepDescription) };
  }

  if (matchesAny(text, ['select', 'choose', 'pick'])) {
    return { type: 'select', target: cleanTarget(stepDescription), value: testData };
  }

  if (matchesAny(text, ['verify', 'validate', 'check', 'assert', 'confirm', 'ensure', 'wait for'])) {
    return { type: 'validate', target: cleanTarget(stepDescription), value: testData || null };
  }
  ...
  if (matchesAny(text, ['hover', 'mouse over'])) {
    return { type: 'hover', target: cleanTarget(stepDescription), value: null };
  }

  if (matchesAny(text, ['upload', 'attach', 'browse for'])) {
    return { type: 'upload', target: cleanTarget(stepDescription), value: testData };
  }

  // Generic fallback – try clicking
  var target = cleanTarget(stepDescription);
  return { type: 'click', target: target, value: null };
}
